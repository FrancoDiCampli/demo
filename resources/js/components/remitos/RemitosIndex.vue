<template>
  <div>
    <template>
      <v-data-table hide-actions :headers="headers" :items="data.remitos" :loading="inProcess">
        <v-progress-linear v-slot:progress color="primary" indeterminate></v-progress-linear>
        <template v-slot:items="remito">
          <td>
            <div>{{ remito.item.id }}</div>
          </td>
          <td class="hidden-sm-and-down">{{ remito.item.supplier_id }}</td>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Proveedor", sortable: false, class: "hidden-sm-and-down" },
